...
import { Loader2, ArrowLeft, User, Trophy, CheckCircle2, XCircle, CalendarClock } from 'lucide-react';
  ...
  const load = async () => {
    try {
      ...
      const { data: enr, error: eErr } = await supabase
        ...
        .select('progress, completed, enrolled_at')
      ...
      const { data: attemptsData, error: aErr } = await supabase
        ...
        .select('id, attempt_number, passed, score_percent, score_numeric, started_at, submitted_at, mooc_exams(id,title,course_id)')
        .eq('user_id', studentId)
        ...
        <Card>
          ...
          <CardContent>
            {attempts.length === 0 ? (
              ...
              <div className="space-y-2">
                {attempts.map((a) => (
                  <div key={a.id} className="grid gap-2 md:grid-cols-5 items-center border rounded p-2">
                    ...
                    <div>
                      <div className="text-sm">{a.score_percent ?? 0}%</div>
                      <div className="text-xs text-muted-foreground">{a.passed ? 'Aprobado' : 'No aprobado'}</div>
                    </div>
                    <div>
