...
create table if not exists public.mooc_exams (
  id uuid primary key default gen_random_uuid(),
  course_id uuid not null references public.mooc_courses(id) on delete cascade,
  lesson_id uuid references public.mooc_lessons(id) on delete set null,
  title text not null,
  description text,
  order_index integer default 0,
  status text not null default 'draft' check (status in ('draft','published','archived')),
  max_score numeric(4,2) not null default 5.00,
  passing_score numeric(4,2) not null default 3.00,
...
create table if not exists public.mooc_exam_questions (
  id uuid primary key default gen_random_uuid(),
  exam_id uuid not null references public.mooc_exams(id) on delete cascade,
...
create table if not exists public.mooc_exam_options (
  id uuid primary key default gen_random_uuid(),
  question_id uuid not null references public.mooc_exam_questions(id) on delete cascade,
  text text not null,
...
create table if not exists public.mooc_exam_answers (
  id uuid primary key default gen_random_uuid(),
  attempt_id uuid not null references public.mooc_exam_attempts(id) on delete cascade,
  question_id uuid not null references public.mooc_exam_questions(id) on delete cascade,
  selected_option_id uuid references public.mooc_exam_options(id) on delete set null,
...
-- Updated timestamps function and triggers
...
drop trigger if exists trg_mooc_exams_updated on public.mooc_exams;
create trigger trg_mooc_exams_updated
before update on public.mooc_exams
for each row execute function public.set_updated_at();

drop trigger if exists trg_mooc_exam_questions_updated on public.mooc_exam_questions;
create trigger trg_mooc_exam_questions_updated
before update on public.mooc_exam_questions
for each row execute function public.set_updated_at();

drop trigger if exists trg_mooc_exam_attempts_updated on public.mooc_exam_attempts;
create trigger trg_mooc_exam_attempts_updated
before update on public.mooc_exam_attempts
for each row execute function public.set_updated_at();

drop trigger if exists trg_mooc_exam_answers_updated on public.mooc_exam_answers;
...
drop trigger if exists trg_attempt_limit on public.mooc_exam_attempts;
...
create or replace function public.compute_exam_score(p_attempt_id uuid)
...
drop trigger if exists trg_compute_score_on_submit on public.mooc_exam_attempts;
...
-- Drop existing policies before creating
drop policy if exists "exams_select_creator_or_published_enrolled" on public.mooc_exams;
drop policy if exists "exams_insert_creator_or_admin" on public.mooc_exams;
drop policy if exists "exams_update_delete_owner_or_admin" on public.mooc_exams;
drop policy if exists "questions_select_visible" on public.mooc_exam_questions;
drop policy if exists "questions_cud_owner_or_admin" on public.mooc_exam_questions;
drop policy if exists "options_select_visible" on public.mooc_exam_options;
drop policy if exists "options_cud_owner_or_admin" on public.mooc_exam_options;
drop policy if exists "attempts_select_owner_or_instructor" on public.mooc_exam_attempts;
drop policy if exists "attempts_insert_enrolled_published" on public.mooc_exam_attempts;
drop policy if exists "attempts_update_owner_before_submit" on public.mooc_exam_attempts;
drop policy if exists "answers_select_owner_or_instructor" on public.mooc_exam_answers;
drop policy if exists "answers_insert_owner_before_submit" on public.mooc_exam_answers;
drop policy if exists "answers_update_owner_before_submit" on public.mooc_exam_answers;
...
for select to authenticated using (
  exists (
    select 1 from public.mooc_exams ex where ex.id = exam_id and (
      ex.created_by = auth.uid() or has_role(auth.uid(), 'admin'::app_role)
...
for select to authenticated using (
  user_id = auth.uid() or exists (
    ...
    where ex.id = exam_id and (c.created_by = auth.uid() or has_role(auth.uid(), 'admin'::app_role))
  )
);
...
for select to authenticated using (
  exists (
    ...
    where a.id = attempt_id and (
      a.user_id = auth.uid() or c.created_by = auth.uid() or has_role(auth.uid(), 'admin'::app_role)
    )
  )
);
